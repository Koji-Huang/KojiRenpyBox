...
renpy.register_shader("draw_rect", 
    ...
    fragment_300="""
        ...
        if(0.0 < uv.x && uv.x < u_rect_area.z && 0.0 < uv.y && uv.y < u_rect_area.w) {
            val = 1.0 - max(abs(uv.x - u_rect_area.z/2) / u_rect_area.z * 2, abs(uv.y - u_rect_area.w/2) / u_rect_area.w * 2);
        }
            
        
        
        if(val > 0){
            if(u_round > 0) val = count_round(uv, u_rect_area.zw, u_round);
...
renpy.register_shader("draw_line_for_transform", 
    variables = """
        attribute vec4 a_position;
        attribute vec2 a_tex_coord;
        uniform vec2 u_model_size;
    ...
    """,
    vertex_300="""
        vec2 sub = u_end_pos-u_start_pos;

        v_line = vec3(u_start_pos, length(sub));
        float degree = atan(sub.x, sub.y) - 3.1415926 / 2;
        vec3 triangle = vec3(sin(degree), cos(degree), degree);

        v_position = a_tex_coord * u_model_size;
        v_position.xy -= v_line.xy;

        mat2 trans = mat2(triangle.y, -triangle.x, triangle.x, triangle.y);
        v_position *= trans;
        v_coord = u_relat > 0.5 ? vec2(gl_Position.x * .5 + .5, -gl_Position.y * .5 + .5) : a_tex_coord;
    """,
    fragment_functions="""
    ...
    fragment_300="""
        vec2 uv = v_position;
        float val = 0.0;
        if(0.0 < uv.x && uv.x < v_line.z && abs(uv.y) < u_width/2) { val = 1.0 - abs(uv.y) / u_width * 2.0;}
        
        if(val > 0.0 && u_round > 0.0) val = count_round(uv, v_line, u_round, u_width, val);
...
renpy.register_shader("draw_rect_for_transform", 
    variables = """
        attribute vec4 a_position;
        attribute vec2 a_tex_coord;
        uniform vec2 u_model_size;
    ...
    fragment_300="""
        ...
        if(0.0 < uv.x && uv.x < u_size.x && 0.0 < uv.y && uv.y < u_size.y) {
            val = 1.0 - max(abs(uv.x - u_size.x/2) / u_size.x * 2, abs(uv.y - u_size.y/2) / u_size.y * 2);
        }
            
        
        if(val > 0){
            if(u_round > 0) val = count_round(uv, u_size, u_round);
            ...
            if(u_width > 0){
                ...
                if(u_round > 0) {
                    ...
                    float x_cost = abs(uv.x - u_size.x/2.0) - u_size.x/2.0 + u_width;
                    float y_cost = abs(uv.y - u_size.y/2.0) - u_size.y/2.0 + u_width;
                    if(x_cost < 0) x_cost = 0.0;
                    else x_cost = 1.0 - abs(x_cost / u_width - 0.5) * 2;
                    if(y_cost < 0) y_cost = 0.0;
                    else y_cost = 1.0 - abs(y_cost / u_width - 0.5) * 2;
                    if(abs(uv.y - u_size.y/2.0) + u_width/2 > (u_size.y) / 2) x_cost *= y_cost;
                    if(abs(uv.x - u_size.x/2.0) + u_width/2 > (u_size.x) / 2) y_cost *= x_cost;
...
renpy.register_shader("draw_circle_for_transform", 
    variables = """
        attribute vec4 a_position;
        attribute vec2 a_tex_coord;
        uniform vec2 u_model_size;
    ...
    fragment_300="""
        ...
        float degree = atan(uv.x, uv.y)-u_offset;
        degree = fract(degree / 3.1415926 / 2.0) * 3.1415926 * 2; 
        ...
        if(val > 0.0 && u_radius < 3.1415926 * 2.0){
            if(0 < degree && degree < u_radius) {
                if(u_width > 0.0)
                    val = count_round(
                        vec2((degree) * u_r, u_r-dis), 
                        abs(u_radius) * u_r, 

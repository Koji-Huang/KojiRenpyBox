...
> Fixed at: 2026.5.23 Koji-Huang  
> - 纠正了 Markdown 语法使用不当的问题
> - 修正了部分内容的表述不当
> - 删去了部分重复的内容
...
**注意:** 
`super().__init__()` 是必须的
此语句用于调用父组件的初始化函数, 不懂的话可以百度`python super`这个词条
不然会爆出一个很无厘头的错误
...
3. **Screen Language 语句**

就是在 rpy 里面编写 screen 时使用的各种代表着组件的语句, 如何给一个可视组件注册到 Screen Language 的内容将在 `Stage6` 进行讲解 
...
def render(self, width, height, st, at):
    ...
    调用以显示此可视化组件。
    被调用时会传入宽度和高度参数，它们给出了这个可视化组件可以绘制到的最大宽度和高度，而不会溢出一边界框。
    也会传入两个时间戳
...
我们可以通过 `renpy.Render(w, h)`来获取一个空白的 `Render` 对象,  `w, h` 为此对象应该占据的高 ( 但实际上我们绘制的范围是可以超出此宽高的, 这两个参数有他用 ), 在获取时此对象是完全透明的
...
在这个教程里, 我先不讲 `event` 函数的返回值具有什么意义 ( 实际上我也没利用过这个机制 ), 而是重点聚焦在前面的第一个参数 `ev`
...
`ev.type` 为 `int` 值, `Pygame` 里存在一些常量用于确认对应的类型, 比如 `pygame.MOUSEBUTTONDOWN = 1025`  
...
这个不难理解, 我们可以用 `ev.__dict__` 把 `ev` 的属性打印出来, 就比如 type 为 `MOUSEBUTTONDOWN` 的事件的 `__dict__` 为:
...
在这里我们可以通过 `button` 参数来获取此 `MOUSEBUTTONDOWN` 事件中鼠标按下的按钮为哪一个, 其他参数也各自有各自的用处

注意, 不同的 `ev` 甚至是同个 `type` 的 `ev` 都会有不一样的属性(例如滚轮滑动)
...
> 返回一个`Render`对象，此对象是对原`Render`对象的剪裁。
> -  `rect`
...
# Stage 6: 注册 Screen Language 语句快速调用组件

在上面的内容里, 我们已经可以很容易的编写出自己的可视化组件了, 但我们在调用时会特别麻烦, 此时我们就需要用更好的方法来调用自己编写的组件, 也就是本节的内容

## a.  什么是 Screen Language 语句
官网的文档链接: https://doc.renpy.cn/zh-CN/screen_python.html#creator-defined-sl

简单来说就是我们可以通过这个方法来将可视化组件注册为 `Renpy` 脚本可以识别的关键字, 我认为官网的文档写的足够健全了, 请浏览完官网文档的内容再看下面的内容吧 ( 决不是偷懒, 是我自己的表达能力实在是没官网的好 )

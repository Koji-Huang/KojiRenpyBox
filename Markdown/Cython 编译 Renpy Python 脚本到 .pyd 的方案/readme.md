# Cython 编译 Renpy Python 脚本到 .pyd 的方案
...
如果你不希望自己提取这份库文件的话, 可以下载我提取的版本(在 Markdown 的目录里面), 但是如果你希望自己提取一份, 请参照此处的教程  
如果你 Renpy 中用的是 3.12 的 Python 那就用 3.12 的 lib, 别用混了 ( 8.4.0 后用的就是 3.12 )
...
运行顺利的话, 会生成 `aaa.xxx.pyd` 的文件, 此时就大功告成了

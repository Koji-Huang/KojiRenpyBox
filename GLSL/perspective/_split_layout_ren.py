...
class SplitLayout(renpy.display.layout.Container):
    def __init__(self, 
        ...
        self.render_limit = (render_limit, render_limit) if type(render_limit) == bool else render_limit
        self.event_limit = (event_limit, event_limit) if type(event_limit) == bool else event_limit
    ...
    def render(self, width, height, st, at):
        ...
        if len(self.children) >= 2:
            if self.render_limit[1]:
                render_b = render(self.children[1], *b_render_size, st, at)
            else:
                render_b = render(self.children[1], width, height, st, at)
    ...
    def event(self, ev, x, y, st):
        ...
        if (x > b_pos[0] and self.direction[0]) or (y > b_pos[1] and self.direction[1]):
            ...
            if len(self.children) >= 1 and self.event_limit[0] is False:
            ...
            if len(self.children) >= 1:
                # print("A", self.children[0], st)
                self.children[0].event(ev, x, y, st)
            if len(self.children) >= 2 and self.event_limit[1] is False:
                # print("B", self.children[1], st)
                self.children[1].event(ev, x - b_pos[0], y - b_pos[1], st)
